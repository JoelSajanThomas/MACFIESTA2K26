/**
 * Comprehensive seed list of prominent Colleges, Universities, and Schools across Kerala.
 * Merged dynamically with any live API / past registered institutions.
 */
export const SEED_INSTITUTIONS = [
  // =========================================================================
  // --- PATHANAMTHITTA DISTRICT & CENTRAL TRAVANCORE COLLEGES ---
  // =========================================================================
  "Mar Athanasios College for Advanced Studies Tiruvalla (MACFAST)",
  "Titus II Teachers College, Tiruvalla",
  "Mar Thoma College, Tiruvalla",
  "Believers Church Medical College Hospital, Thiruvalla",
  "Pushpagiri Institute of Medical Sciences, Tiruvalla",
  "Pushpagiri College of Pharmacy, Tiruvalla",
  "Pushpagiri College of Nursing, Tiruvalla",
  "Pushpagiri College of Dental Sciences, Medicity, Perumthuruthy",
  "Pushpagiri College of Allied Health Sciences, Tiruvalla",
  "St. Thomas College, Kozhencherry",
  "Catholicate College, Pathanamthitta",
  "N.S.S. College, Pandalam",
  "St. Cyril's College, Adoor",
  "College of Engineering, Adoor",
  "College of Engineering, Kallooppara",
  "College of Engineering, Aranmula",
  "Mount Zion College of Engineering, Kadammanitta",
  "Musaliar College of Engineering and Technology, Pathanamthitta",
  "Musaliar Institute of Management, Pathanamthitta",
  "College of Applied Science (IHRD), Pathanamthitta",
  "College of Applied Science (IHRD), Mallappally",
  "College of Applied Science (IHRD), Konni",
  "College of Applied Science (IHRD), Adoor",
  "Government Arts & Science College, Elanthoor",
  "Government Polytechnic College, Vennikulam",
  "Government Polytechnic College, Adoor",
  "Government College of Teacher Education, Kozhencherry",
  "SAS SNDP Yogam College, Konni",
  "BAM College, Thuruthicad, Mallappally",
  "St. Thomas College, Ranny",
  "Caarmel Engineering College, Ranni, Perunad",
  "Mar Severios College of Teacher Education, Chengaroor",
  "Mannam Memorial N.S.S. College of Teacher Education, Pandalam",
  "SNDP Yogam Arts & Science College, Kadammanitta",
  "Ayyankali Memorial Arts and Science College, Kulanada, Pandalam",
  "St. Thomas College of Advanced Studies, Edamuri, Ranni",
  "St. Gregorios College of Teacher Education, Meenangadi",
  "Bishop Abraham Memorial College, Thuruthicad",
  "NSS Polytechnic College, Pandalam",
  "St. Thomas College of Teacher Education, Mylapra",
  "UIT (University Institute of Technology), Pathanamthitta",
  "UIT (University Institute of Technology), Adoor",
  "UIT (University Institute of Technology), Malayalappuzha",
  "UIT (University Institute of Technology), Mannady",
  "UIT (University Institute of Technology), Vakayar, Konni",

  // =========================================================================
  // --- KOTTAYAM & CHANGANASSERY COLLEGES ---
  // =========================================================================
  "St. Berchmans College (SB College), Changanassery",
  "Assumption College, Changanassery",
  "NSS Hindu College, Changanassery",
  "CMS College, Kottayam",
  "Baselius College, Kottayam",
  "Bishop Chulaparambil Memorial College (BCM), Kottayam",
  "Government College, Kottayam",
  "Rajiv Gandhi Institute of Technology (RIT), Kottayam",
  "Amal Jyothi College of Engineering, Kanjirappally",
  "Saintgits College of Engineering, Pathamuttom",
  "Saintgits Institute of Management, Kottayam",
  "Mangalam College of Engineering, Ettumanoor",
  "St. Thomas College, Pala",
  "Alphonsa College, Pala",
  "St. Joseph's College of Engineering and Technology, Palai",
  "St. Dominic's College, Kanjirapally",
  "Henry Baker College, Melukavu",
  "K.G. College, Pampady",
  "Deva Matha College, Kuravilangad",
  "BVM Holy Cross College, Cherpunkal",
  "Ettumanoorappan College, Ettumanoor",
  "College of Applied Science (IHRD), Puthuppally",
  "College of Applied Science (IHRD), Kanjirappally",
  "College of Applied Science (IHRD), Kaduthuruthy",
  "Marian College, Kuttikkanam (Autonomous)",
  "Mar Augusthinose College, Ramapuram",
  "Kristu Jyoti College of Management and Technology, Chethipuzha",
  "Girideepam Institute of Advanced Learning, Kottayam",
  "Government College, Nattakom, Kottayam",

  // =========================================================================
  // --- ALAPPUZHA & CHENGANNUR COLLEGES ---
  // =========================================================================
  "Christian College, Chengannur",
  "College of Engineering, Chengannur",
  "SN College, Chengannur",
  "Sree Ayyappa College, Eramallikara",
  "Bishop Moore College, Mavelikara",
  "Raja Ravi Varma College of Fine Arts, Mavelikara",
  "Pezhakkappilly College of Engineering, Chengannur",
  "MSM College, Kayamkulam",
  "Sanatana Dharma College (SD College), Alappuzha",
  "St. Joseph's College for Women, Alappuzha",
  "T.K. Madhava Memorial College (TKMM), Nangiarkulangara",
  "St. Michael's College, Cherthala",
  "N.S.S. College, Cherthala",
  "Sree Narayana College, Cherthala",
  "College of Engineering, Cherthala",
  "College of Applied Science (IHRD), Mavelikkara",
  "College of Applied Science (IHRD), Punnappra",
  "College of Applied Science (IHRD), Karthikappally",
  "Carmel College of Engineering & Technology, Punnapra",
  "St. Aloysius College, Edathua",
  "Government College, Ambalapuzha",

  // =========================================================================
  // --- KOLLAM COLLEGES ---
  // =========================================================================
  "TKM College of Engineering, Kollam",
  "TKM College of Arts & Science, Kollam",
  "Fatima Mata National College, Kollam",
  "Sree Narayana College, Kollam",
  "Sree Narayana College for Women, Kollam",
  "Sree Narayana Institute of Technology (SNIT), Adoor",
  "St. Gregorios College, Kottarakkara",
  "St. Stephen's College, Pathanapuram",
  "St. John's College, Anchal",
  "NSS College, Nilamel",
  "Sree Narayana College, Punalur",
  "Sree Narayana College, Chathannur",
  "MMNSS College, Kottiyam",
  "Baby John Memorial Government College, Chavara",
  "Government Arts & Science College, Thazhava",
  "KSMDB College, Sasthamcotta",
  "College of Engineering, Karunagappally",
  "College of Engineering, Kottarakkara",
  "College of Engineering, Perumon",
  "College of Applied Science (IHRD), Kundara",
  "College of Applied Science (IHRD), Dhanuvachapuram",
  "Aenis College of Engineering, Kollam",
  "Travancore Engineering College, Oyoor",
  "Marthoma College of Science and Technology, Ayur",

  // =========================================================================
  // --- THIRUVANANTHAPURAM COLLEGES ---
  // =========================================================================
  "College of Engineering Trivandrum (CET)",
  "Government Engineering College, Barton Hill",
  "University College, Thiruvananthapuram",
  "Mar Ivanios College, Nalanchira",
  "Government College for Women, Thiruvananthapuram",
  "Mahatma Gandhi College, Kesavadasapuram",
  "All Saints' College, Thiruvananthapuram",
  "St. Xavier's College, Thumba",
  "Government College, Kariavattom",
  "Government Arts College, Thiruvananthapuram",
  "Government Sanskrit College, Thiruvananthapuram",
  "Government College, Attingal",
  "Government College, Nedumangad",
  "Mar Baselios College of Engineering and Technology (MBCET)",
  "LBS Institute of Technology for Women, Poojappura",
  "Mohandas College of Engineering and Technology",
  "Marian Engineering College, Kazhakuttam",
  "SCT College of Engineering, Pappanamcode",
  "Sarabhai Institute of Science and Technology, Vellanad",
  "Trinity College of Engineering, Trivandrum",
  "Sree Narayana College, Chempazhanthy",
  "Sree Narayana College, Sivagiri, Varkala",
  "VTM NSS College, Dhanuvachapuram",
  "Christian College, Kattakada",
  "IIST - Indian Institute of Space Science and Technology",
  "IISER Thiruvananthapuram",
  "Sree Chitra Thirunal College of Engineering",
  "Loyola College of Social Sciences, Sreekariyam",

  // =========================================================================
  // --- ERNAKULAM / KOCHI COLLEGES ---
  // =========================================================================
  "Maharaja's College, Ernakulam",
  "St. Teresa's College (Autonomous), Ernakulam",
  "St. Albert's College, Ernakulam",
  "Sacred Heart College (SH College), Thevara",
  "Rajagiri College of Social Sciences, Kalamassery",
  "Rajagiri School of Engineering & Technology (RSET), Kakkanad",
  "Cochin University of Science and Technology (CUSAT)",
  "Model Engineering College (MEC), Thrikkakara",
  "Bharata Mata College, Thrikkakara",
  "St. Paul's College, Kalamassery",
  "Union Christian College (UC College), Aluva",
  "St. Xavier's College for Women, Aluva",
  "Federal Institute of Science and Technology (FISAT), Angamaly",
  "SCMS School of Engineering and Technology, Karukutty",
  "Adi Shankara Institute of Engineering and Technology, Kalady",
  "Sree Sankara College, Kalady",
  "Sree Sankaracharya University of Sanskrit, Kalady",
  "Mar Athanasius College (Autonomous), Kothamangalam",
  "Mar Athanasius College of Engineering (MACE), Kothamangalam",
  "Nirmala College, Muvattupuzha",
  "Ilahia College of Engineering and Technology, Muvattupuzha",
  "Cochin College, Kochi",
  "Aquinas College, Edakochi",
  "MES College, Marampally, Aluva",
  "St. Peter's College, Kolenchery",
  "Baselios Poulose II Catholicos College, Piravom",
  "Albertian Institute of Science and Technology (AISAT)",
  "Toc H Institute of Science and Technology, Arakkunnam",
  "Muthoot Institute of Technology and Science (MITS), Varikoli",
  "De Paul Institute of Science & Technology (DiST), Angamaly",
  "Amrita School of Arts and Sciences, Kochi",
  "Amrita Vishwa Vidyapeetham, Edappally",

  // =========================================================================
  // --- THRISSUR & CENTRAL KERALA COLLEGES ---
  // =========================================================================
  "Government Engineering College, Thrissur (GECT)",
  "St. Thomas College (Autonomous), Thrissur",
  "St. Aloysius College, Elthuruth, Thrissur",
  "Vimala College (Autonomous), Thrissur",
  "St. Mary's College, Thrissur",
  "Sree Kerala Varma College, Thrissur",
  "Christ College (Autonomous), Irinjalakuda",
  "St. Joseph's College, Irinjalakuda",
  "Prajyoti Niketan College, Pudukad",
  "Sree Narayana College, Nattika",
  "Panampilly Memorial Government College, Chalakudy",
  "Sacred Heart College, Chalakudy",
  "Vidya Academy of Science and Technology, Thrissur",
  "Jyothi Engineering College, Cheruthuruthy",
  "IES College of Engineering, Chittilappilly",
  "Royal College of Engineering & Technology, Akkikavu",
  "Sahrdaya College of Engineering and Technology, Kodakara",
  "Sahrdaya College of Advanced Studies, Kodakara",
  "Kerala Agricultural University (KAU), Vellanikkara",
  "Kerala University of Health Sciences (KUHS)",

  // =========================================================================
  // --- IDUKKI & PALAKKAD COLLEGES ---
  // =========================================================================
  "Government Engineering College, Idukki",
  "NSS College of Engineering, Palakkad",
  "Government Victoria College, Palakkad",
  "Mercy College, Palakkad",
  "NSS College, Ottapalam",
  "Sree Narayana College, Alathur",
  "Government College, Chittur",
  "IIT Palakkad",
  "Newman College, Thodupuzha",
  "St. Joseph's College, Moolamattom",
  "Government College, Kattappana",
  "Government College, Munnar",

  // =========================================================================
  // --- MALAPPURAM, KOZHIKODE, WAYANAD, KANNUR & KASARAGOD COLLEGES ---
  // =========================================================================
  "National Institute of Technology Calicut (NITC)",
  "Government Engineering College, Kozhikode",
  "Farook College (Autonomous), Kozhikode",
  "St. Joseph's College, Devagiri, Kozhikode",
  "Providence Women's College, Kozhikode",
  "Malabar Christian College, Kozhikode",
  "Zamorin's Guruvayurappan College, Kozhikode",
  "Government Arts & Science College, Meenchanda, Kozhikode",
  "Government Engineering College, Wayanad",
  "MES College of Engineering, Kuttippuram",
  "Government College, Malappuram",
  "EMEA College of Arts and Science, Kondotty",
  "PSMO College, Tirurangadi",
  "MES Mampad College",
  "Government College of Engineering, Kannur (GCEK)",
  "Sir Syed College, Taliparamba",
  "Brennen College, Thalassery",
  "Sree Narayana College, Kannur",
  "Payyanur College, Payyanur",
  "Nehru Arts and Science College, Kanhangad",
  "Government College, Kasaragod",
  "LBS College of Engineering, Kasaragod",
  "Central University of Kerala, Kasaragod",

  // =========================================================================
  // --- SCHOOLS IN PATHANAMTHITTA DISTRICT (Comprehensive) ---
  // =========================================================================
  // Thiruvalla
  "Believers Church Residential School (BCRS), Thiruvalla",
  "Scared Heart Higher Secondary School, Thiruvalla",
  "MGM Higher Secondary School, Thiruvalla",
  "St. Mary's Residential Central School, Thiruvalla",
  "Christ Central School, Muthoor, Thiruvalla",
  "The Choice School, Thiruvalla",
  "Mar Thoma Residential School (MTRS), Tiruvalla",
  "Balikamadom Higher Secondary School, Thiruvalla",
  "St. Thomas Higher Secondary School, Thiruvalla",
  "Devalokam Higher Secondary School, Thiruvalla",
  "Govt Boys Higher Secondary School, Thiruvalla",
  "Govt Girls Higher Secondary School, Thiruvalla",
  "NSS Higher Secondary School, Kaviyoor, Thiruvalla",
  "St. John's Higher Secondary School, Eraviperoor, Thiruvalla",
  "YMCA Centenary School, Thiruvalla",
  "Seventh Day Adventist Higher Secondary School, Thiruvalla",
  "St. Paul's Higher Secondary School, Muthoor, Thiruvalla",
  "NSS High School, Kuttoor, Thiruvalla",

  // Kozhencherry, Ranny, Mallappally
  "St. Thomas Higher Secondary School, Kozhencherry",
  "Govt Higher Secondary School, Kozhencherry",
  "St. Mary's Higher Secondary School, Kozhencherry",
  "St. Thomas Higher Secondary School, Ranny",
  "Citadel Residential School, Ranni",
  "Govt Higher Secondary School, Ranny",
  "MS Higher Secondary School, Ranny",
  "Bethany Higher Secondary School, Ranni",
  "St. Mary's High School, Anaprampal, Ranni",
  "CMS Higher Secondary School, Mallappally",
  "St. Thomas Higher Secondary School, Eraviperoor",
  "St. George Higher Secondary School, Kottangal, Mallappally",
  "Govt Higher Secondary School, Kunnamthanam, Mallappally",
  "NSS Higher Secondary School, Kunnamthanam",
  "BAM Higher Secondary School, Thuruthicad",
  "St. Teresa's Higher Secondary School, Chengaroor, Mallappally",

  // Adoor, Pandalam, Pathanamthitta
  "Kendriya Vidyalaya, Adoor",
  "Holy Angels English Medium Higher Secondary School, Adoor",
  "St. Cyril's Higher Secondary School, Adoor",
  "Govt Boys Higher Secondary School, Adoor",
  "Govt Girls Higher Secondary School, Adoor",
  "St. Mary's Higher Secondary School, Adoor",
  "Tapovanam Residential School, Adoor",
  "N.S.S. Higher Secondary School, Pandalam",
  "Govt Higher Secondary School, Pandalam",
  "St. Thomas Higher Secondary School, Pandalam",
  "Govt Higher Secondary School, Thumpamon, Pandalam",
  "Marthoma Higher Secondary School, Pathanamthitta",
  "Catholicate Higher Secondary School, Pathanamthitta",
  "Govt Boys Higher Secondary School, Pathanamthitta",
  "Govt Girls Higher Secondary School, Pathanamthitta",
  "St. Peter's Higher Secondary School, Jnanapeedam, Pathanamthitta",
  "SNDP Higher Secondary School, Chenneerkkara, Pathanamthitta",
  "Navodaya Vidyalaya, Vechoochira, Pathanamthitta",
  "Govt Higher Secondary School, Omalloor",
  "St. Mary's Higher Secondary School, Niranam",

  // Konni & Aranmula
  "Govt Higher Secondary School, Konni",
  "St. George Higher Secondary School, Attachakkal, Konni",
  "Govt Higher Secondary School, Kalanjoor, Konni",
  "Govt Higher Secondary School, Aranmula",
  "NSS Higher Secondary School, Choorakode",
  "Govt Higher Secondary School, Mezhuveli",
  "St. Thomas Higher Secondary School, Kadampanad",

  // =========================================================================
  // --- PROMINENT SCHOOLS ACROSS OTHER DISTRICTS IN KERALA ---
  // =========================================================================
  // Kottayam & Changanassery Schools
  "St. Berchmans Higher Secondary School, Changanassery",
  "Assumption High School, Changanassery",
  "Placid Vidya Vihar Senior Secondary School, Chethipuzha",
  "Kristu Jyoti Higher Secondary School, Chethipuzha",
  "Pallikoodam School, Kalathilpady, Kottayam",
  "Girideepam Bethany Central School, Kottayam",
  "Marian Senior Secondary School, Kottayam",
  "Lourdes Public School and Junior College, Kottayam",
  "Mount Carmel Higher Secondary School, Kottayam",
  "St. Ephrem's Higher Secondary School, Mannanam",
  "St. Joseph's Higher Secondary School, Manarcad",
  "Baker Memorial Girls Higher Secondary School, Kottayam",
  "MDT Hindu Higher Secondary School, Kottayam",
  "Don Bosco Higher Secondary School, Puthuppally",
  "St. Antony's Higher Secondary School, Kanjirappally",
  "St. Mary's Higher Secondary School, Pala",
  "St. Thomas Higher Secondary School, Pala",
  "Kendriya Vidyalaya, Rubber Board, Kottayam",
  "Jawahar Navodaya Vidyalaya, Vadavathoor, Kottayam",

  // Alappuzha & Kollam Schools
  "St. Anne's Higher Secondary School, Chengannur",
  "Govt Boys Higher Secondary School, Chengannur",
  "Govt Girls Higher Secondary School, Mavelikkara",
  "Bishop Moore Vidyapith, Mavelikara",
  "Bishop Moore Vidyapith, Cherthala",
  "St. Aloysius Higher Secondary School, Edathua",
  "St. Joseph's Girls Higher Secondary School, Alappuzha",
  "Leo XIII Higher Secondary School, Alappuzha",
  "SDV Higher Secondary School, Alappuzha",
  "Jawahar Navodaya Vidyalaya, Chennithala, Alappuzha",
  "Infant Jesus Anglo Indian Higher Secondary School, Tangasseri, Kollam",
  "St. Aloysius Higher Secondary School, Kollam",
  "Trinity Lyceum, Kollam",
  "Mount Tabor Girls Higher Secondary School, Pathanapuram",
  "St. Gregorios Higher Secondary School, Kottarakkara",
  "St. John's Higher Secondary School, Anchal",
  "St. John's Higher Secondary School, Mattom",

  // Thiruvananthapuram Schools
  "Loyola School, Sreekariyam, Thiruvananthapuram",
  "St. Thomas Residential School, Mukkolakkal, Thiruvananthapuram",
  "St. Thomas Central School, Mukkolakkal, Thiruvananthapuram",
  "Sarvodaya Vidyalaya, Nalanchira, Thiruvananthapuram",
  "Arya Central School, Pattom, Thiruvananthapuram",
  "Kendriya Vidyalaya, Pattom, Thiruvananthapuram",
  "Kendriya Vidyalaya, Pangode, Thiruvananthapuram",
  "Army Public School, Pangode, Thiruvananthapuram",
  "Sainik School, Kazhakootam, Thiruvananthapuram",
  "Holy Angels' Convent Higher Secondary School, Thiruvananthapuram",
  "Christ Nagar Higher Secondary School, Kowdiar, Thiruvananthapuram",
  "St. Mary's Higher Secondary School, Pattom, Thiruvananthapuram",

  // Ernakulam / Kochi Schools
  "Bhavans Vidya Mandir, Girinagar, Kochi",
  "Bhavans Vidya Mandir, Elamakkara, Kochi",
  "Bhavans Adarsha Vidyalaya, Kakkanad",
  "Bhavans Varuna Vidyalaya, Thrikkakara",
  "The Choice School, Thripunithura, Kochi",
  "Rajagiri Christu Jayanthi Public School, Kakkanad",
  "Rajagiri Public School, Kalamassery",
  "Chinmaya Vidyalaya, Vaduthala, Kochi",
  "Chinmaya Vidyalaya, Tripunithura",
  "Kendriya Vidyalaya, Port Trust, Kochi",
  "Kendriya Vidyalaya, INS Dronacharya, Kochi",
  "Viswajyothi CMI Public School, Angamaly",
  "Mar Athanasius International School (MAIS), Kothamangalam",
  "Nirmala Public School, Muvattupuzha",
  "Marygiri English Medium Higher Secondary School, Koothattukulam",
  "Vimal Jyothi Public School, Muppathadam",
  "Jawahar Navodaya Vidyalaya, Neriamangalam, Ernakulam",

  // Thrissur, Kozhikode, Kannur & Others
  "Hari Sri Vidya Nidhi School, Thrissur",
  "Kulapati Munshi Bhavans Vidya Mandir, Pottore, Thrissur",
  "St. Thomas College Higher Secondary School, Thrissur",
  "Don Bosco Higher Secondary School, Irinjalakuda",
  "Don Bosco Higher Secondary School, Mannuthy",
  "Chinmaya Vidyalaya, Thrissur",
  "Kendriya Vidyalaya, Puranattukara, Thrissur",
  "Jawahar Navodaya Vidyalaya, Mayannur, Thrissur",
  "The Choice School, Kozhikode",
  "Devagiri CMI Public School, Kozhikode",
  "Silver Hills Public School, Kozhikode",
  "Presentation Higher Secondary School, Kozhikode",
  "Sadhbhavana World School, Kozhikode",
  "Kendriya Vidyalaya, Kozhikode",
  "Chinmaya Vidyalaya, Kannur",
  "Kendriya Vidyalaya, Kannur",
  "De Paul Higher Secondary School, Thodupuzha",
  "Jai Rani Higher Secondary School, Thodupuzha",
];

export const OTHERS_VALUE = "__others__";
export const OTHERS_LABEL = "Others (not in list)";

export function mergeInstitutions(extra = []) {
  const set = new Map();
  [...SEED_INSTITUTIONS, ...extra].forEach((raw) => {
    const name = String(raw || "").trim().replace(/\s+/g, " ");
    if (name.length < 2) return;
    if (name.toLowerCase() === "macfast") return;
    const key = name.toLowerCase();
    if (!set.has(key)) set.set(key, name);
  });
  return [...set.values()].sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }));
}

export function filterInstitutions(list, query, limit = 20) {
  const q = String(query || "").trim().toLowerCase();
  if (!q) return list.slice(0, limit);
  const starts = [];
  const contains = [];
  for (const name of list) {
    const lower = name.toLowerCase();
    if (lower.startsWith(q)) {
      starts.push(name);
    } else if (lower.includes(q)) {
      contains.push(name);
    }
    if (starts.length + contains.length >= limit * 3) break;
  }
  return [...starts, ...contains].slice(0, limit);
}
