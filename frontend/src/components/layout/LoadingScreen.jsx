...
export default function LoadingScreen({ skip = false }) {
  ...
  useEffect(() => {
    if (!isMounted || isDismissed || isDone) return undefined;
  ...
  }, [isMounted, isDismissed, isDone]);

  // ── Sequence: intro → reveal ─────────────────────────────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    if (phase !== "reveal" || burstFiredRef.current || skip || isDone) return undefined;
  ...
  if (!isMounted || skip || isDone || isDismissed) return null;
